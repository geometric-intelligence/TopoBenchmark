...
class PreProcessor(torch_geometric.data.InMemoryDataset):
    ...
    Parameters
    ----------
    dataset : torch_geometric.data.Dataset or torch.utils.data.Dataset or torch_geometric.data.Data
        The dataset to be preprocessed.
    data_dir : str
        Path to the directory containing the data.
    transforms_config : DictConfig, optional
        Configuration parameters for the transforms (default is None).
    **kwargs : dict
    ...
    def instantiate_pre_transform(self, data_dir, transforms_config) -> torch_geometric.transforms.Compose:
        """
    ...
    def set_processed_data_dir(self, pre_transforms_dict, data_dir, transforms_config) -> None:
        ...
        transforms_parameters = {
            transform_name: transform.parameters for transform_name, transform in pre_transforms_dict.items()
        }
    ...
    def save_transform_parameters(self) -> None:
        ...
        """
        path_transform_parameters = os.path.join(self.processed_data_dir, "path_transform_parameters_dict.json")
    ...
    def load_dataset_splits(self, split_params) -> tuple[DataloadDataset, DataloadDataset | None, DataloadDataset | None]:
        ...
        if split_params.learning_setting == "inductive":
            ...
            raise ValueError(f"Invalid '{split_params.learning_setting}' learning setting. Please define either 'inductive' or 'transductive'.")
