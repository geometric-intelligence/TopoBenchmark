...
def compare_models(results, p_limit=0.05, verbose=False):
    ...
    if friedman_result>p_limit:
        if verbose:
            print(f"The Friedman test confirmed the null-hypothesis with p-value of {friedman_result}")
    ...
    for i in range(M):
        ...
        while i+idx<M:
            ...
            if verbose:
                print(f"P-value for Wilcoxon test between models {model_idx} and {next_model_idx}: {p}")
            if p<p_limit:
                break
...
if __name__ == '__main__':
    results = np.array([[0.7,0.7,0.8,0.9,0.7,0.6,0.8,0.9,0.7,0.9],[0.6,0.65,0.7,0.9,0.5,0.552,0.843,0.78,0.665,0.876]])
    print(signed_ranks_test(results[0,:],results[1,:]))
    results2 = np.array([[0.7,0.7,0.8,0.9,0.7,0.6,0.8,0.9,0.7,0.9],[0.69,0.71,0.77,0.9,0.59,0.811,0.813,0.78,0.712,0.899],[0.1,0.2,0.2,0.3,0.4,0.5,0.22,0.32,0.11,0.4]])
    print(friedman_test(results2))
    results3 = np.array([[0.9,0.9,0.8,0.8,0.7],[0.91,0.89,0.81,0.79,0.71],[0.89,0.91,0.79,0.81,0.69]])
