    ...
    def __init__(self, metrics=[]):
        ...
        for m in metrics:
            if m not in accepted_metrics:
                raise ValueError(f"Metric {m} is not valid. Choose betweeen {accepted_metrics}.")
    ...
    def eval(self, input_dict):
        ...
        for metric in self.metrics:    
            ...
            if metric == 'acc':
                ...
                results["pre_micro"] = precision_score(res_true, res_pred, average="micro")
                results["pre_macro"] = precision_score(res_true, res_pred, average="macro")
            elif metric == 'rec':
