...
def k_fold_split(labels, parameters):
    ...
    # Check that all nodes/graph have been assigned to some split
    assert (
        np.unique(np.array(split_idx["train"].tolist() + split_idx["valid"].tolist() + split_idx["test"].tolist())).shape[0]
        == labels.shape[0]
...
def random_splitting(
    labels, parameters, global_data_seed=42
):
    ...
    if generate_splits:
        ...
        for fold_n in range(10):
            ...
            split_idx = {"train": train_indices, "valid": val_indices, "test": test_indices}
...
def load_graph_tudataset_split(dataset, cfg):
    ...
    assert len(dataset) > 1, "Torch Geometric TU datasets should have more than one graph in the dataset"
