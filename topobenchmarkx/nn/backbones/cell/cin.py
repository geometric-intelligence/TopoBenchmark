...
class CWN(torch.nn.Module):
    ...
    def forward(
        ...
        neighborhood_2_to_1 : torch.Tensor
...
class CWNLayer(nn.Module):
    ...
    conv_1_to_1 : torch.nn.Module, optional
        ...
        and their corresponding co-boundary (r+1) cells (default: None).
    conv_0_to_1 : torch.nn.Module, optional
        A module that convolves the representations of (r-1)-cells on the boundary of r-cells (default: None).
    aggregate_fn : torch.nn.Module, optional
        A module that aggregates the representations of r-cells obtained by convolutional layers (default: None).
...
class _CWNDefaultFirstConv(nn.Module):
    ...
    ----------
    in_channels_1 : int
        Dimension of input features on r-cells.
    in_channels_2 : int
        Dimension of input features on (r+1)-cells.
    out_channels : int
        Dimension of output features on r-cells.
    eps : float, optional
        A learnable parameter that scales the input features before the first convolutional layer.
    """
